...
TEST(PowellModel, Regression) {
  ...
  moptimizer::Cost<double, Eigen::Vector4d, powell> cost(nullptr, nullptr, 1);
  ...
  for (int i = 0; i < 25; ++i) {
    cost.linearize<parameter_dimension>(parameters.data(), hessian_numeric.data(), b_numeric.data());
